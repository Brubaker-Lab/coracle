...
run_test <- function(use_future = NULL) {
  ...
  test_that("corr_col() for two data frames", {
    ...
    test_that("erroneous inputs for joining", {
      expect_error(suppressMessages(f(
        x = data.frame(up), y = data.frame(up)
      ))) # Only column used for joining
      expect_error(suppressMessages(f(
        x = data.frame(indexes, up),
        y = data.frame(indexes, up, down)
      ))) # All columns in x used for joining
      expect_error(suppressMessages(f(
        x = data.frame(indexes, up, down),
        y = data.frame(indexes, up)
  ...
  # test_that("corr_col() with `future` and large data frames", {
  #   expect_true(exists("corr_col")) # Prevents "Empty Test" warning
  #
  #   f <- corr_col
  #
  ...
  #
  #     test_that("large data frames", {
  #       expect_snapshot(suppressMessages(f(large_df)))
  #       expect_snapshot(suppressMessages(f(large_df, large_df)))
  #     })
  #   } else {
  #     future::plan(future::sequential)
  #   }
...
run_test(use_future = FALSE)
run_test(use_future = TRUE)
